                ...
                SpecialInstruction::Pack => {
                    ...
                    if !has_more_packed {
                        ...
                        write_impl.extend(
                            quote_spanned!(
                                serialize_type_span =>
                                bytes = <graphite_binary::slice_serialization::Single as SliceSerializable<u8>>::write(bytes,
                                    #inner_write_impl
                                );
                            )
                        );
                        ...
                        read_impl.extend(
                            quote_spanned!(
                                serialize_type_span =>
                                let #packed_ident = <graphite_binary::slice_serialization::Single as SliceSerializable<u8>>::read(bytes)?;
                            )
                        );
            ...
            Input::Enum {
                ...
                for (index, variant) in variants.iter().enumerate() {
                    ...
                    write_impl.extend(
                        quote!(
                            Self::#ident { #enum_struct_fields } => {
                                bytes = <graphite_binary::slice_serialization::Single as graphite_binary::slice_serialization::SliceSerializable<u8>>::write(bytes, #index);
                                #variant_write_impl
                                bytes
                            }
                        )
