    ...
    fn get_successor(&mut self, x: i32, y: i32, z: i32, dist: f32, max_steps: i32) -> Successor {
        ...
        if cost.value == f32::INFINITY {
            ...
            Successor {
                ...
                y: y+steps,
                z,
                move_cost: cost.value + dist + steps as f32 * 0.5
            }
        } else {
            let mut steps = 0;
            loop {
                ...
                if cost.is_swimmable {
                    if steps <= 3 {
                        cost.value += steps as f32 * 0.25;
                    } else {
                        cost.value += (steps as f32 - 3.0) * 2.0;
                    }
                    ...
                    return Successor {
                        ...
                        y: y-steps,
                        z,
                        move_cost: cost.value + dist
                    };
                }

                let below_cost = self.evaluator.get_combined_cost(x, y-steps-1, z, self.entity_width, self.entity_height);
                if below_cost.value == f32::INFINITY {
                    if steps <= 3 {
                        cost.value += steps as f32 * 0.5;
                    } else {
                        cost.value += (steps as f32 - 3.0) * 4.0;
                    }
                ...
                cost.value += below_cost.value;
                cost.is_swimmable = below_cost.is_swimmable;
                steps += 1;
            }
        }
    }
...
impl <E: NodeCostEvaluator> PathNodeTraverser for LandPathNodeTraverser<E> {
    ...
    fn get_successors(&mut self, x: i32, y: i32, z: i32, successors: &mut Vec<Successor>) {
        ...
        let max_steps = if above_cost.value == f32::INFINITY {
            0
        } else {
            1
        };
        ...
        let self_cost = self.evaluator.get_combined_cost(x, y, z, self.entity_width, self.entity_height);
        if self_cost.is_swimmable {
            if above_cost.value != f32::INFINITY && above_cost.is_swimmable {
                successors.push(Successor {
                    x,
                    y: y+1,
                    z,
                    move_cost: above_cost.value + 0.25,
                });

                for edge in [(1, 1), (-1, 1), (1, -1), (-1, -1)] {
                    let edge_cost = self.evaluator.get_combined_cost(x+edge.0, y+1, z+edge.1,
                        self.entity_width, self.entity_height);
                    if edge_cost.value != f32::INFINITY && edge_cost.is_swimmable {
                        successors.push(Successor {
                            x: x + edge.0,
                            y: y + 1,
                            z: z + edge.1,
                            move_cost: above_cost.value + edge_cost.value + 1.1,
                        });
                    }
                }
            }

            let below_cost = self.evaluator.get_combined_cost(x, y-1, z, self.entity_width, self.entity_height);
            if below_cost.value != f32::INFINITY && below_cost.is_swimmable {
                successors.push(Successor {
                    x,
                    y: y-1,
                    z,
                    move_cost: below_cost.value + 0.25,
                });
                ...
                for edge in [(1, 1), (-1, 1), (1, -1), (-1, -1)] {
                    ...
                    if edge_cost.value != f32::INFINITY && edge_cost.is_swimmable {
                        successors.push(Successor {
                            ...
                            y: y - 1,
                            z: z + edge.1,
                            move_cost: below_cost.value + edge_cost.value + 1.1,
                        });
                    }
                }
            }
