...
pub fn brigadier(attr: TokenStream, item: TokenStream) -> TokenStream {
        ...
        match attribute {
            ...
            BrigadierAttribute::Argument { span: _, modifiers } => {
                ...
                let parser_node = quote!(
                    Some(graphite_command::minecraft::MinecraftArgumentNode {
    ...
    let parse_function = quote!(
        fn #command_identifier_parse(data: &[u8], spans: &[graphite_command::types::Span]) -> graphite_command::types::CommandDispatchResult {
            #[repr(C)]
...
fn process_num_arg(
    ...
    Ok((
        true,
        quote!(
            graphite_command::minecraft::NumericParser::#parser_typ {
                min: #min_expr,
                max: #max_expr
            }
        ),
        quote!(
            if data.#deconstruct_index < #min_expr {
                return graphite_command::types::CommandDispatchResult::ParseError {
