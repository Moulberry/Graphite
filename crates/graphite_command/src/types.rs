use std::{alloc::Layout, borrow::Cow, result};
...
impl<'a> ParseState<'a> {
    ...
    pub(crate) fn remaining_span(&self) -> Span {
        if self.cursor > 0 {
            Span {
                start: self.words[self.cursor - 1].span.start,
                end: self.full_span.end
            }
        } else {
            self.full_span
        }
    ...
    pub fn push_str(&mut self, arg: &str, span: Span) {
        let raw_str: u128 = unsafe { std::mem::transmute(arg) };
        self.push_arg_with_layout(raw_str, span, Layout::new::<&str>());
    }

    pub fn push_ref<T>(&mut self, arg: &T, span: Span) {
        let raw_reference: usize = unsafe { std::mem::transmute(arg) };
        self.push_arg_with_layout(raw_reference, span, Layout::new::<&T>());
    }
    ...
    pub fn push_arg<T: NoUninit>(&mut self, arg: T, span: Span) {
        ...
        // Update layout
        self.push_arg_with_layout(arg, span, arg_layout);
    }

    fn push_arg_with_layout<T: NoUninit>(&mut self, arg: T, span: Span, arg_layout: Layout) {
        // Update layout
        ...
        assert_eq!(arg_layout.size(), bytes.len());
...
pub type DispatchFunction<T> = fn(&mut T, &[u8], &[Span]) -> CommandDispatchResult;
pub type SuggestionsFunction = fn(&str) -> Vec<Cow<'_, str>>;
...
pub enum ParseErrorType {
    FloatTooBig { max: String, found: String },
    FloatTooSmall { min: String, found: String },
    ExpectedFloat,
    IntegerTooBig { max: String, found: String },
    IntegerTooSmall { min: String, found: String },
    ExpectedInteger,
    ExpectedBoolean,
    Other(String)
}
