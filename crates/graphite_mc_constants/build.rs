...
mod entities;
#[path = "build/particle.rs"]
mod particle;
#[path = "build/item.rs"]
mod item;
#[path = "build/builtin.rs"]
mod builtin;
#[path = "build/types.rs"]
mod types;
#[path = "build/font_width.rs"]
mod font_width;
...
fn main() {
    ...
    let builtin_result = builtin::write_registries().unwrap();

    let (_placement_method_returns, _aliases, block_name_to_state) = block::write_block_states().unwrap();
    fluid::write_fluid_states().unwrap();
    item::write_items(block_name_to_state, builtin_result.data_component_types).unwrap();
    entities::write_entities().unwrap();
    particle::write_particles().unwrap();
    ...
    types::write_types().unwrap();
    font_width::write_font_widths().unwrap();
