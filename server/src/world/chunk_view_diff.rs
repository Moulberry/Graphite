...
where
    ...
    F2: FnMut(i32, i32)
{
    for_each_diff_with_min_max(delta, view_distance, new_chunks, old_chunks, 
        i32::MIN, i32::MIN, i32::MAX, i32::MAX)
}

#[inline(always)] // We want to be able to fold min_x, etc.
pub fn for_each_diff_with_min_max<F1, F2>(delta: (i32, i32), view_distance: u8, mut new_chunks: F1, mut old_chunks: F2, 
        min_x: i32, min_z: i32, max_x: i32, max_z: i32)
where
    F1: FnMut(i32, i32),
    F2: FnMut(i32, i32)
{
    debug_assert!(1 <= view_distance && view_distance <= 16, "view distance must be between 1 and 16");
...
mod tests {
    ...
    quickcheck! {
        ...
        fn quickcheck_chunk_diff_with_min_max(input: u32) -> bool {
    ...
    fn simple_chunk_diff() {
        ...
        for_each_diff(delta, view_distance as _, |x, z| {
            let index = expected_new_chunks.iter().position(|v| *v == (x, z))
                .expect(&format!("for_each_diff thought {},{} was a new chunk, but it isn't", x, z));
            expected_new_chunks.remove(index);
        }, |x, z| {
            let index = expected_old_chunks.iter().position(|v| *v == (x, z))
                .expect(&format!("for_each_diff thought {},{} was a old chunk, but it isn't", x, z));
            expected_old_chunks.remove(index);
        });

        debug_assert_eq!(expected_old_chunks.len(), 0, "for_each_diff failed to provide values for old: {:?}", expected_old_chunks);
        debug_assert_eq!(expected_new_chunks.len(), 0, "for_each_diff failed to provide values for new: {:?}", expected_new_chunks);
    }
